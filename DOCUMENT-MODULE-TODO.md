...
- [x] **۲.۵ — پشتیبانیِ `TemplateSection` (چندصفحه با اندازهٔ متفاوت) در کامندها** ✅
  ...
  - **تصمیم (با تأییدِ مالکِ محصول): «فقط قابلِ‌دیدن کن».** هیچ بخشی از مدل نباید برای لایهٔ
    کامند نامرئی بماند (همان اصلِ ۱.۱)، ولی UIِ سکشن‌دار فازِ جداست.
  - **انجام‌شده:** `findElement` حالا داخلِ `sections[].bands` را هم می‌گردد و
    `ElementLocation.sectionIndex` را گزارش می‌کند (برای عنصرِ سطحِ بالا حاضر نیست، نه
    `undefined`). `mapBands` هر دو فهرست را می‌پیماید و سکشن‌های دست‌نخورده را referentially
    یکسان نگه می‌دارد. `findBand`/`replaceBand` اضافه شدند و **همهٔ کامندهای باند** از آن‌ها
    استفاده می‌کنند، پس باندِ داخلِ سکشن هم patch/remove/move می‌شود؛ `addBand`/`moveBand`
    پارامترِ اختیاریِ `sectionIndex` گرفتند. `moveElementZ` هم باندش را با id پیدا می‌کند نه
    `bands[index]`، وگرنه zهای سطحِ بالا روی سکشن اثر می‌گذاشت.
  - **کامندهای سکشن:** `addSection`/`removeSectionById`/`patchSection`/`moveSection`.
    `patchSection` عمداً `bands` را نمی‌گیرد (`Omit<…,'bands'>`) چون باندها کامندهای خودشان
    را دارند.
  - دامنهٔ دیزاینر در کامنتِ `model/template.ts` صریح شد: ویرایشگرِ بصری یک پشتهٔ باندِ تخت را
    ویرایش می‌کند؛ ساختنِ سندِ چندسکشنی فعلاً کارِ سطحِ API است.
  - ۲۵ تست، از جمله دو تست که ثابت می‌کنند ویرایش واقعاً به **رندر** می‌رسد (تغییرِ عنصرِ داخلِ
    سکشن، و جابه‌جاییِ سکشن که تنظیماتِ صفحه را با خودش می‌برد).
