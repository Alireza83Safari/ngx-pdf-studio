...
- [x] **۲.۶ — تستِ round-trip روی fixtureِ فاکتورِ واقعی** ✅
  - assert: N فیلد، جدولِ اقلام، شکلِ `sampleData`؛ import→render شبیهِ اصل.
  - **انجام‌شده:** `clone-roundtrip.spec.ts` — یک فاکتورِ بایندشده را به **PDFِ واقعی** رندر می‌کند، با **pdfjs** (همان مسیرِ دکمهٔ دیزاینر) دوباره استخراج و `cloneFormat` می‌کند، بعد assert: برچسب‌های سربرگ → فیلدِ بایند (`invoice_no`,`date`, مقدار `INV-1024`)؛ خطوطِ اقلام → **یک جدولِ بایندشده با ۳ ستون و ۳ ردیف**؛ شکلِ `inferredData`؛ و **رندرِ دوبارهٔ کلون مقادیرِ اصل را بازتولید می‌کند** (`INV-1024`,`2024-05-01`,`Widget`,`Sprocket`). **۴۷۲ تستِ core سبز** + typecheck/lint تمیز.
